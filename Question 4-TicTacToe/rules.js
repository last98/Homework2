...
function begin_play(){
	var p1Input = document.getElementById("player1_id");
	var p2Input = document.getElementById("player2_id");
	var rule1pass = (p1Input.value.length > 0 && p2Input.value.length > 0);
...
*/
function play() {
	var inputMove = document.getElementById("move_text_id");
		//variable for whatever user enters into text box/field
	var turnInfo = document.getElementById("turn_info");
	...
	if(table_ids.includes(inputMove.value)){
		//if input value is a valid move (A1-C3)
		var i = table_ids.indexOf(inputMove.value);
			//index variable for index at which user wants to place an X or O
		console.log("This is i: " + i);
			//testing to see if index is correct
		bstate = this.board_state[i];
			//integer at that index
		if(this.turn){
			//if it's player 1s turn, now switch to player 2
			//var i = table_ids.indexOf(inputMove.value);
			//console.log("This is i: " + i);
			//bstate = this.board_state[i]; 
			if(bstate === -1){
				...
				//update element at that index to indicate space has been taken
				document.getElementById(inputMove.value).innerHTML = "X";
				//update html at that index to X
				if(winner()){
					//winner found
					alert("Winner is X");
					reset_play();
			...
			//if it's player 2s turn, now switch to player 1
				//var i = table_ids.indexOf(inputMove.value);
				//bstate = this.board_state[i];
				if(bstate === -1){
					turnInfo.innerHTML = "Turn for : <b>X</b> ";
					this.board_state[i] = 0;
					document.getElementById(inputMove.value).innerHTML = "O";
					if(winner()){
						alert("Winner is O");
						reset_play();
					}
		...
		alert("Please enter in A1, A2, A3, B1, B2, B3, C1, C2, or C3.");
	}
}
